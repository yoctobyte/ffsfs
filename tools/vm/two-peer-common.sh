...
two_peer_boot_and_sync() {
    two_peer_boot_vm "$name_a" "$ssh_a" "$peer_a_host_port"
    two_peer_boot_vm "$name_b" "$ssh_b" "$peer_b_host_port"
...
'
    ...
    sleep 2
    two_peer_collect_guest_debug "$ssh_a" "$name_a"
    two_peer_collect_guest_debug "$ssh_b" "$name_b"
}
